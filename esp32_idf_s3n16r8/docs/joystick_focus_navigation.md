...
交换方向。当前实机配置为 X 不反转、Y 反转，因此物理向上对应
`GUI_INPUT_UP`。
...
自动息屏/休眠只把确认过的按键按下视为持续活动。启动时已经为低电平的
主按钮或 B，以及被中立保护拒绝的 B 按下，都不会重置空闲计时或误唤醒。
按键释放并再次有效按下后恢复正常确认、长按和唤醒功能，避免 GPIO21
持续低电平导致 `idle=0ms`、电台永不息屏。
