...
- 自动休眠（永远不 / 5 / 10 / 30 分钟）和自动息屏（永远不 / 1 / 5 /
  10 分钟），在设置中切换并保存到 SPIFFS；任意按钮或摇杆动作唤醒。

## 自动省电

设置列表向下滚动可找到「自动休眠」和「自动息屏」，按确认循环切换，
保存成功后立即生效，重启后保留；默认均为「永远不」。
两个计时器均从最后一次按钮或摇杆操作计算，息屏不会重新开始休眠计时，
同时到期时休眠优先。持续按住按钮或推动摇杆不计为空闲。

休眠会关闭显示、停止录音与所有当前播放、取消未完成的语音回复、退出游戏
和配置热点，仅保留 RTC 走时、整点报时及输入唤醒。整点报时可以临时启用音频，
但不会点亮屏幕。息屏只关闭显示并退出游戏，电台和其他音频继续；
息屏期间整点报时会暂时中断电台，结束后恢复原电台。
报时仍遵循现有 07:00–23:00 时段和安静模式设置。

唤醒后不会自动恢复被休眠终止的播放或游戏；用于唤醒的整次操作（包括松开和
摇杆连发）不会触发录音、确认或游戏动作，松手/摇杆回中后可正常操作。

**背光接线限制：** 固件使用 ST7789 的 DISPOFF/SLPIN 命令并停止界面刷新，
不是仅绘制黑色。当前接线没有可控背光 GPIO；若背光直接接电源，软件无法关闭
背光 LED。要进一步省电，需要将模块的背光控制端按电气规格接至空闲 GPIO
（必要时使用驱动电路），并配置 `APP_TFT_BACKLIGHT_PIN` 和
`APP_TFT_BACKLIGHT_ON`。这里的休眠是应用层休眠，不是 ESP32 深度睡眠。
